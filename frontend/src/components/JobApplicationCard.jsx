...
const JobApplicationCard = ({ application }) => {
  const { jobTitle, location, appliedDate, currentStage, status ,companyName} = application;
  ...
  const stageColorClass = (stageName, currentStageName, status) => {
  ...
  return (
    <div className="bg-white shadow sm:rounded-md mb-4 p-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <div>
          <p className="text-sm font-semibold">Stage - {currentStage}</p>
        ...
        <div className="grid grid-cols-4 gap-4">
          {stages.map((stage, index) => (
            <div key={index} className="text-center">
              ...
              <div className={`w-6 h-6 mx-auto rounded-full ${stageColorClass(stage, currentStage, status)}`}></div>
            </div>
          ))}
        </div>
      </div>
