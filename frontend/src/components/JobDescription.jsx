    ...
    const fetchJobAndCheckApplication = async () => {
      ...
      try {
        ...
        if (response.data) {
          const jobData = {
            ...response.data,
            created_at: new Date(parseInt(response.data._id.substring(0, 8), 16) * 1000) // Convert the timestamp
          };
          setJob(jobData);
        }
        if (userId) {
        ...
        <div className="md:grid md:grid-cols-3">
          <div className="md:col-span-2">
            ...
            <div className="job-icons px-4 py-3">
              <JobIcon icon="fas fa-building" text={job.company_id ? job.company_id.name : 'Not specified'} />
              <JobIcon icon="fas fa-briefcase" text={job.experience || 'Not specified'} />
              <JobIcon icon="fas fa-map-marker-alt" text={job.location_id ? job.location_id.location_name : 'Not specified'} />
              <JobIcon icon="fas fa-sitemap" text={job.department_id ? job.department_id.name : 'Not specified'} />
              <JobIcon icon="far fa-clock" text={`Posted on ${job.created_at.toDateString()}`} />
            </div>
          ...
          <div className="md:col-span-1 p-6">
            {!userId ? (
              ...
              <div className="py-3 font-bold">You have already applied for this job.</div>
            ) : (
              ...
              <table className="table-fixed w-full">
                <tbody>
                  <tr className="border-b">
                    <td className="py-2 text-gray-600 font-medium">Company</td>
                    <td className="py-2 text-gray-900">{job.company_id.name}</td>
                  </tr>
                  <tr className="bg-gray-100 border-b">
                    <td className="py-2 text-gray-600 font-medium">Type</td>
                    <td className="py-2 text-gray-900">{job.job_type}</td>
                  </tr>
                  <tr className="border-b">
                    <td className="py-2 text-gray-600 font-medium">Location</td>
                    <td className="py-2 text-gray-900">{job.location_id ? job.location_id.location_name : 'Not specified'}</td>
                  </tr>
...
//           const jobData = {
//             ...response.data,
//             created_at: new Date(parseInt(response.data._id.substring(0, 8), 16) * 1000)
//           };
//           setJob(jobData);
//         }
//         if (userId) {
